...
bitová identita s originálním datasetem: **aktuálně neověřitelná** bez soukromého artefaktu
(dataset/seed/config nebyly veřejně vydány; jejich definitivní zánik není prokázán) — náš pilot
...
| sémantika solveru | **~95 %** | supplement p.7: DeepStack „CFR+" = hybrid (RM+, simultánní updaty, uniformní vážení, omit raných iterací) = sémantika našeho enginu; skip existence je AUTHOR-CONFIRMED; exact count 500 zůstává RELEASED-CODE-ANCHORED, při konvergenci na 1000 it. vliv na cíle řádově setiny % potu |
...
| vidlička | stav | plán |
|---|---|---|
| offline omit (500?) | HIGH-CONFIDENCE RELEASED-CODE-ANCHORED | senzitivní experiment: identické vstupy, omit 0/250/500, delta cílů v % potu |
| bin [100,100) | singleton (3× koroborace) | senzitivita: podíl vzorků a vliv na trénink při [100,200) čtení |
| tie-breaking síly | PROJECT CANONICAL (high-impact: equal-strength ties frequently cross recursive split boundaries) | dokumentováno; testovatelné permutačním experimentem |
...
## 2026-08-16 author update
Martin Schmid's direct recollection closes the binary question of whether the
training-target solver used a skip window: **yes, skip iterations were used**.
The exact private HUNL count is still unresolved; project value `500` remains
RELEASED-CODE-ANCHORED to the author DeepStack-Leduc pipeline.
