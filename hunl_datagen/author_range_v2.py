...
def current_public_hand_strength(board) -> HandStrengthResult:
    ...
    No future public cards are dealt.  For a turn board this ranks the six-card
    holdings (2 private + 4 public) using the certified ACPC ``rankCardset``
    evaluator semantics.  Ties are not wins, because the source says
    ...
    if len(b) not in (3, 4):
        raise ValueError("range-generation public board must be flop or turn")
    if len(set(b)) != len(b):
